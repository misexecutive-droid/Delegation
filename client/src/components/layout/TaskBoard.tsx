...
const Column = ({ status, tasks, assigneeNames, departmentNames, isVerifier, onOpen, onAddTask, fields }: ColumnProps) => {
  ...
  return (
    <div
      ref={setNodeRef}
      className={`flex flex-col gap-2 rounded-xl border p-2 transition-colors duration-150 shrink-0 w-[82vw] sm:w-[300px] md:w-auto snap-center md:snap-align-none ${
...
export const TaskBoard = ({ tasks, assigneeNames, departmentNames, isVerifier = false, onOpen, onAddTask, fields }: TaskBoardProps) => {
  ...
  return (
    ...
    >
      <div className="flex md:grid md:grid-cols-4 gap-3 md:gap-4 items-start overflow-x-auto pb-4 snap-x snap-mandatory no-scrollbar">
        {COLUMNS.map(status => (
          <Column
            key={status}
            status={status}
