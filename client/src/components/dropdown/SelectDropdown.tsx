...
const TRIGGER_CLASS =
  ...
  'transition-all outline-none cursor-pointer hover:border-border-hover ' +
  'focus-visible:ring-2 focus-visible:ring-primary-500/20 focus-visible:border-primary-400 ' +
...
export function SelectDropdown<Value extends string>({
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align={align} className={cn('min-w-[8rem]', contentClassName)}>
        {options.map((opt) => (
          <DropdownMenuItem
            key={opt.value}
